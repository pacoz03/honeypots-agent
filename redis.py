...
class RedisHoneyPotDeployTool(BaseTool):
    """Un tool per deployare l'honeypot RedisHoneyPot da un Dockerfile."""
    name: str = "redishoneypot_deployer"
    description: str = (
        "Usa questo tool per deployare o aggiornare RedisHoneyPot. "
        "Prepara i file da 'config_source/redishoneypot' nella directory 'honeypots/redishoneypot' "
        "ed esegue 'docker build' e 'docker run'. Questo tool forza l'aggiornamento del container."
    )

    # --- Costanti di classe per i percorsi ---
    HONEYPOT_TYPE: ClassVar[str] = "redishoneypot"
    CONTAINER_NAME: ClassVar[str] = "redishoneypot"
    
    SOURCE_DIR: ClassVar[str] = os.path.join("config_source", HONEYPOT_TYPE)
    SOURCE_CONFIG_DIR: ClassVar[str] = os.path.join(SOURCE_DIR, "dist")
    ...
    def _run(self, *args, **kwargs) -> str:
        ...
        try:
            ...
            print(f"[TOOL] Running docker-compose from '{self.DEPLOY_DIR}'...")
            ...
            success_message = (
                ...
                f"Il servizio è esposto sulla porta 6379. Per vedere i log, esegui: 'docker logs {self.CONTAINER_NAME}'."
            )
    ...
    def _copy_files_to_deployment_dir(self):
        ...
        print(" - Docker Compose copiato con successo.")

        # Copia i file di configurazione nella sottocartella 'dist'
        shutil.copy(os.path.join(self.SOURCE_CONFIG_DIR, "redis.conf"), self.DEPLOY_CONFIG_DIR)
